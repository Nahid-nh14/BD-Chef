...
const ChefDetails = lazy(()=> import('./ChefDetails'))
...
const Recipes = () => {
    ...
    const {recipes} = chefDatas
    

    useEffect(() => {
        const fetchData = async () => {
            const res = await fetch('https://bd-chef-data-nahid-nh14.vercel.app/data')
            const datas = await res.json()
            const chef =  datas.find(data => data.id == id)
...
// recipes.map((data) => )
                // recipes.map(data=> console.log(data))
...
{
// recipes.map((data) => console.log(data))
    // recipes.map((data , i) => console.log(data)
    // <RecipesDetails
    // key={i}
    // data={data}
    ...
    // ></RecipesDetails>
    // )
//     recipes.map((data, i) => <div
//      key={i}
    
//     > {data.name} 
    
//     <ul>
//         {data.ingredients.map((d,i) => <li key={i}>{d.name}</li> )
// }
//     </ul>
